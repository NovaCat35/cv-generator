...
export default function AdditionalForm({ isActive, onExpand, currState, setState, handleRemoveChange, handleSubmitCategory }: AdditionalFormProps) {
	...
	const removeBulletCard = (e: React.MouseEvent<HTMLButtonElement>) => {
		...
		if (targetID !== null) {
			// Remove bulletPt from bulletPoint list
			const newSkillList = currMainInfo.bulletPoints.filter((bullet) => bullet.id !== targetID);

			// Remove bulletPt also from subheader's bulletPointIds
			const updatedSubheaderList = currMainInfo.subHeaders.map((subheader) => {
				const updatedBulletIDList = subheader.bulletPointIds.filter((bulletPtID) => bulletPtID !== targetID);
				return { ...subheader, bulletPointIds: updatedBulletIDList };
			});

			// Remove subheader if no more bulletPt IDs in bulletPointIds list
			const remainingSubheaders = updatedSubheaderList.filter((subheader) => subheader.bulletPointIds.length > 0);

			// Update state with new values
			setCurrMainInfo((prevState) => ({
				...prevState,
				bulletPoints: newSkillList,
				subHeaders: remainingSubheaders,
			}));
		} else {
